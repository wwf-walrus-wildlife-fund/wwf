...
import { datasets, categories } from "./mockData";
    ...
    <div className="relative min-h-screen pt-24 pb-16 px-4">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div
          ...
          className="mb-10"
        >
          <h1 className="text-white mb-2" style={{ fontSize: "2rem" }}>Marketplace</h1>
          <p className="text-white/30">Browse, buy, or rent data from verified providers.</p>
        </motion.div>
            ...
            <div className="flex gap-2">
              ...
              <div className="hidden sm:flex items-center rounded-xl bg-white/[0.03] border border-white/[0.08] overflow-hidden">
                <button
                  ...
                  className={`p-3 transition-colors ${viewMode === "grid" ? "text-[#C4B5FD] bg-[#4834D4]/10" : "text-white/30 hover:text-white/50"}`}
                >
                  <Grid3X3 className="w-4 h-4" />
                </button>
                <button
                  onClick={() => setViewMode("list")}
                  className={`p-3 transition-colors ${viewMode === "list" ? "text-[#C4B5FD] bg-[#4834D4]/10" : "text-white/30 hover:text-white/50"}`}
                >
        ...
        {filteredDatasets.length === 0 && (
          <div className="text-center py-20">
            <div className="text-white/10 mb-4" style={{ fontSize: "3rem" }}>:/</div>
            <p className="text-white/30">No datasets match your search.</p>
            <p className="text-white/15 mt-1" style={{ fontSize: "0.85rem" }}>Try adjusting your filters or search terms.</p>
          </div>
        )}
      </div>
    </div>
